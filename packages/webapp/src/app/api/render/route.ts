...
const MAX_SCALE = 6;
const MAX_SLIDES = 20;
const VALID_FORMATS = new Set(["png", "webp", "pdf", "pptx"]);
...
async function generateImagePptx(
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let slides = await page.$$(selector);
    if (slides.length === 0) {
      await browser.close();
      return jsonResponse(
    ...
    if (body.slideRange) {
      const [start, end] = body.slideRange;
      const s = Math.max(1, start) - 1;
      const e = Math.min(slides.length, end);
      slides = slides.slice(s, e);
    }
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (safeFormats.includes("png")) {
        ...
        if (safeFormats.includes("pptx") && !safeFormats.includes("webp")) {
      ...
      if (safeFormats.includes("pptx") && !pptxImages[i]) {
        const buf = await slides[i].screenshot({
          type: "png",
          encoding: "binary",
        });
    ...
    if (safeFormats.includes("pptx") && pptxImages.length > 0) {
      const pptxBuf = await generateImagePptx(
        pptxImages,
        width * scale,
        height * scale,
      );
      const pptxName = body.pptxFilename || "carousel.pptx";
      zip.file(pptxName, pptxBuf);
    }
